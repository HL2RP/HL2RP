	...
	const char* pUserName, const char* pPassword, int port)
{
	if (sqlite3_open(pDatabaseName, &mpConnection) == SQLITE_OK)
	{
		if (IsRelease())
		{
			...
			// to the main file each N-byte blocks. As such, we only enable WAL in release mode, since this file
			// is unsupported by inspection tools and it can take some time for the main file to be updated.
