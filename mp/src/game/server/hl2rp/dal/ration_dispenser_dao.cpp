...
public:
	CDispensersLoadDAO()
	{
		mQueryDatabase.AddCollection(RATION_DISPENSER_DAO_COLLECTION_NAME)
			->AddIndexField("map", STRING(gpGlobals->mapname));
	}
};
...
CRecordNodeDTO* CDispenserDAO::AddFields(CRationDispenserProp* pDispenser, CDatabaseNodeDTO& database, bool save)
{
	CRecordNodeDTO* pDispenserData = database.AddCollection(RATION_DISPENSER_DAO_COLLECTION_NAME)
		->AddIndexField("map", STRING(gpGlobals->mapname));
...
{
	AddFields(pDispenser, mSaveDatabase, true);
}

bool CDispenserInsertDAO::MergeFrom(IDAO* pDAO)
{
	return (pDAO->As(this)->mhDispenser == mhDispenser);
}
...
CDispensersSaveDeleteDAO::CDispensersSaveDeleteDAO(CRationDispenserProp* pDispenser, bool save) : CSaveDeleteDAO(false)
{
	AddFields(pDispenser, GetCorrectDatabase(save), save)
