...
unsigned int CDAL::CDAOHashFunctor::operator()(const CDAOTypeRef& typeRef) const
{
...
int CDAL::Run()
{
	...
	// After each DAO iteration, we apply a small delay, consistent with the configured tickrate for optimization.
	...
	{
		{
			...
			{
				// We're at last DAO from an old level shutdown. Since database configuration
				// would reload upon level change, we can't process any more DAO with current driver.
...
CDatabaseConnection* CDAL::LoadDatabaseConfiguration()
{
	...
	if (!configuration->LoadFromFile(filesystem, "cfg/database.cfg")
		&& !configuration->LoadFromFile(filesystem, "cfg/database_default.cfg"))
	{
		Warning("Failed to load database configuration from file: 'cfg/database.cfg'\n");
	}
	else
	{
		...
		if (Q_stricmp(pDriverName, DAL_SQLITE_DRIVER_NAME) == 0)
		{
			...
			filesystem->RelativePathToFullPath_safe("sqlite3_driver" DLL_EXT_STRING,
				"GAMEBIN", driverPath, FILTER_CULLPACK);

			if (Sys_LoadInterface(driverPath, SQLITE3_DRIVER_IMPL_VERSION_STRING, &pModule, &pDriver))
			{
		...
		else if (Q_stricmp(pDriverName, DAL_MYSQL_DRIVER_NAME) == 0)
		{
			...
			char driverPath[MAX_PATH];
			filesystem->RelativePathToFullPath_safe("mysql_driver" DLL_EXT_STRING,
				"GAMEBIN", driverPath, FILTER_CULLPACK);

			if (Sys_LoadInterface(driverPath, MYSQL_DRIVER_IMPL_VERSION_STRING, &pModule, &pDriver))
			{
