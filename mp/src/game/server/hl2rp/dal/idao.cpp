...
ENUM(ESQLRecordAppendFlag,
	AppendNames = 1,
	AppendValues = 2,
	AssignFields = ESQLRecordAppendFlag::AppendValues | ESQLRecordAppendFlag::AppendNames,
	FilterFields = ESQLRecordAppendFlag::AssignFields | 4, // Default to 'AND' separate multiple fields
	FilterFieldsInclusive = ESQLRecordAppendFlag::FilterFields | 8, // Force to 'OR' separate multiple fields
	AutoDeduceValues = ESQLRecordAppendFlag::AssignFields | 16 // Use driver-dependent automatic value deducing format
)
	...
	KeyValues* pLoadedRecord, CRecordNodeDTO* pMatchingRecord)
{
	...
	if (pMatchingRecord->mNormalFieldByName.FieldsEqual(pLoadedRecord))
	{
		...
		{
			destResult.SetField(pField->GetName(), pField->GetString());
		}
	...
	KeyValues* pLoadedRecord, CRecordNodeDTO* pMatchingRecord)
{
	if (pMatchingRecord->mIndexFieldByName.Count() > 0) // Is the record still valid for saving?
	{
		...
		pMatchingRecord->mIndexFieldByName.RemoveAll();
...
bool CAutoIncrementLoadKeyValuesDTOHandler::OnCollectionStarted(CRecordNodeDTO* pCollection, const char* pName)
{
	...
	catch (CDatabaseIOException&)
	{
		// Some loaded record matched the local one
		COMPILE_TIME_ASSERT(true); // Silent PVS-Studio warning
	}
	...
	KeyValues* pLoadedRecord, CRecordNodeDTO* pMatchingRecord)
{
	if (pMatchingRecord->mNormalFieldByName.FieldsEqual(pLoadedRecord))
	{
		if (pLoadedRecord == pFileData)
		{
			pFileData->Clear();
		}
		else
		{
			pFileData->RemoveSubKey(pLoadedRecord);
		}
	...
	const char* pSeparator, const char* pAutoDeducedValueFormat)
{
	...
	if (FBitSet(flags, ESQLRecordAppendFlag::AppendValues))
	{
		...
		else if (field.mType == EFieldDTOType::String)
		{
			mQuery += "?";
			mStringValues.AddToTail(field.mString);
		}
		else
		{
			mQuery.AppendFormat("%s", field.ToString());
		}
	}
...
void CFilteringSQLDTOHandler::AppendRecords(CRecordNodeDTO* pParent, const char* pSeparator, int depth)
{
	...
	FOR_EACH_DICT_FAST((*pParent), i)
	{
		...
		// If record has siblings, it's a leaf node and it doesn't contain normal fields, then we can group it
...
CAutoIncrementInsertDAO::CAutoIncrementInsertDAO(const char* pCollection, const char* pAutoIncrementField,
	...
	mpRecord(mSaveDatabase.AddCollection(pCollection)), mpAutoIncrementField(pAutoIncrementField)
{
