...
void CHL2Roleplayer::PostThink()
{
	...
	if (IsAlive())
	{
		...
		CBaseEntity* pAimingEntity = GetAimingEntity(mAimingEntityDistance);

		if (pAimingEntity != mhAimingEntity)
		{
			mhAimingEntity = pAimingEntity;
			SendAimingEntityHUD();
		}
...
void CHL2Roleplayer::LocalDisplayHUDHint(EPlayerHUDHintType type, const char* pToken)
{
...
void CHL2Roleplayer::HUDThink()
{
	if (IsAlive())
	{
		...
		{
			CLocalizeFmtStr<> message(this);
			int count = ComputeRegionHUD(regionPlayers, message);
			SendHUDMessage(EPlayerHUDType::Communication, message, HL2RP_COMM_HUD_DEFAULT_X_POS,
...
bool CHL2Roleplayer::IsDamageProtected()
{
...
static void HandleAccessChangeCommand(const CCommand& args, bool grant)
{
		...
		|| (pPlayer != NULL && pPlayer->mAccessFlags.IsBitSet(EPlayerAccessFlag::Root)))
	{
		...
		{
			pTarget = ToHL2Roleplayer(pPlayer->mhAimingEntity);
		}
