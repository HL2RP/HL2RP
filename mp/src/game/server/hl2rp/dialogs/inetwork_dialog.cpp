...
INetworkDialog::INetworkDialog(CHL2Roleplayer* pPlayer, const char* pTitleToken, const char* pMessage)
	: mpPlayer(pPlayer), mpTitleToken(pTitleToken)
{
...
CNetworkEntryBox::CNetworkEntryBox(CHL2Roleplayer* pPlayer, const char* pTitleToken, const char* pMessage, int action)
	: INetworkDialog(pPlayer, pTitleToken, pMessage), mAction(action)
{
...
void CNetworkEntryBox::HandleCommand(const CCommand& args)
{
	...
	if (count > 1)
	{
		mpPlayer->mDialogStack[count - 2]->HandleChildCommand(args, mAction);
...
CNetworkMenu::CNetworkMenu(CHL2Roleplayer* pPlayer, const char* pTitleToken, const char* pMessage)
	: INetworkDialog(pPlayer, pTitleToken, pMessage)
{
