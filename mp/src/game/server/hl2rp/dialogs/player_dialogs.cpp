...
CAdminMenu::CAdminMenu(CHL2Roleplayer* pPlayer) : CNetworkMenu(pPlayer, "#HL2RP_Menu_Admin")
{
		...
		if (trace.DidHit())
		{
			if (fabs(trace.plane.normal.z) < 1.0f - DOT_1DEGREE) // Require a vertical surface
			{
				...
				if (pDispenser != NULL)
				{
					...
					mDispenserId = pDispenser->mDatabaseId;
					pDispenser->mDatabaseId.SetForLoading();
	...
	else if (mhDispenser != NULL)
	{
		...
		case EItemAction::SetRations:
		{
			return mpPlayer->PushAndSendDialog(new CNetworkEntryBox(mpPlayer,
				"#HL2RP_Menu_Dispenser_SetRations", "#HL2RP_Menu_Dispenser_SetRations_Msg"));
		}
...
void CDispensersMenu::HandleChildCommand(const CCommand& args, int)
{
	if (mhDispenser != NULL)
	{
		mhDispenser->mRationsAmmo = Q_atoi(args.Arg(1));
		DAL().AddDAO(new CDispensersSaveDAO(mhDispenser));
	}
}
