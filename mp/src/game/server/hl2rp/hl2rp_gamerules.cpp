...
mpPlayerResponseSystem(PrecacheCustomResponseSystem("scripts/player_responses.txt"))
{
	...
	if (mapGroupsKV->LoadFromFile(filesystem, HL2RP_RULES_CONFIG_PATH "mapgroups.txt"))
	{
	...
	// Load player models
	if (modelsKV->LoadFromFile(filesystem, HL2RP_RULES_CONFIG_PATH "models.txt"))
	{
	...
	// Load jobs data
	if (jobsKV->LoadFromFile(filesystem, HL2RP_RULES_CONFIG_PATH "jobs.txt"))
	{
