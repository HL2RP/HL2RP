...
public:
	CHL2RPMainHUD(const char* pElementName) : CHL2RPHUD(pElementName, "MainHUD") {}
};
...
class CHL2RPAimingEntityHUD : public CHL2RPHUD
{
	...
	float mAimingEntityDistance;

public:
	CHL2RPAimingEntityHUD(const char* pElementName) : CHL2RPHUD(pElementName, "AimingEntityHUD") {}
};
...
{
	mhAimingEntity = GetLocalHL2Roleplayer()->GetAimingEntity(mAimingEntityDistance);
}
...
void CHL2RPAimingEntityHUD::Paint()
{
	if (mhAimingEntity != NULL && mAimingEntityDistance < PLAYER_USE_RADIUS)
	{
		...
		if (*text != '\0')
		{
			BaseClass::Paint(text, mhAimingEntity->IsPlayer() ? mPlayerTextColor : mTextColor);
		}
	}
}
...
class CHL2RPRegionHUD : public CHL2RPHUD
{
	...
	CUtlVector<CBasePlayer*> mPlayers;

public:
	CHL2RPRegionHUD(const char* pElementName) : CHL2RPHUD(pElementName, "RegionHUD") {}
};
