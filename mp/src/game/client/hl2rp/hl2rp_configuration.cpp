...
#define HL2RP_CONFIGURATION_USER_DATA_FILE "cfg/user_data.txt"
...
class CForgetHUDHintsDialog : QueryBox
{
...
public:
	CForgetHUDHintsDialog(Panel* pParent) : QueryBox("#GameUI_Confirm", "#HL2RP_ForgetHUDHintsWarning", pParent)
	{
		SetName("ForgetHUDHintsDialog");
		DoModal();
		m_pMessageLabel->SetName("ForgetHUDHintsWarning");
		LoadControlSettings("resource/forgethudhints_dialog.res");
...
class CGameExperiencePage : public PropertyPage
{
	void OnCommand(const char* pCommand) OVERRIDE
	{
		if (Q_strcmp(pCommand, "ForgetHUDHints") == 0)
		{
			new CForgetHUDHintsDialog(this);
			return;
		}
...
public:
	CGameExperiencePage(Panel* pParent) : PropertyPage(pParent, "GameExperience")
	{
		...
		new Button(this, "ForgetHUDHintsButton", "#HL2RP_ForgetHUDHints", this, "ForgetHUDHints");
	}
};

class COtherSettingsDialog : public PropertyDialog
{
	~COtherSettingsDialog()
	{
		gHL2RPConfiguration.mpOtherSettingsDialog = NULL;
	}

public:
	COtherSettingsDialog() : PropertyDialog(FindGameUIChildPanel("BaseGameUIPanel"), "OtherSettings")
	{
		SetDeleteSelfOnClose(true);
		SetVisible(true);
		CGameExperiencePage* pPage = new CGameExperiencePage(this);
		LoadControlSettings("resource/other_settings.res");
...
void CHL2RPConfiguration::PostInit()
{
	...
	{
		char text[64];
		pMainMenu->GetMenuItem(i)->GetText(text, sizeof(text));
		Q_StripPrecedingAndTrailingWhitespace(text);

		if (text[0] == '#')
		{
			pMainMenu->GetMenuItem(i)->SetText(text);
		}
	}
}
...
void CHL2RPConfiguration::CmdDisplayOtherSettingsDialog(const CCommand&)
{
	if (mpOtherSettingsDialog == NULL)
	{
		mpOtherSettingsDialog = new COtherSettingsDialog;
	}

	mpOtherSettingsDialog->Activate();
}
