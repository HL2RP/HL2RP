...
void CRationDispenserProp::GetHUDInfo(CHL2Roleplayer* pPlayer, LOCCHAR_T* pDest, int maxLen)
{
	if (SequenceDuration() <= 0.0f)
	{
		...
		else if (mNextTimeAvailable > gpGlobals->curtime)
		{
			...
			message.Localize("#HL2RP_Dispenser_Restoring", "#HL2RP_Duration_HHMMSS",
				time.mHours, time.mMinutes, time.mSeconds);
		}
		else
		{
			message.Localize("#HL2RP_Dispenser_Available");
