...
void CWeaponShotgun::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator, bool bUseWeaponAngles)
{
	...
	pOperator->FireBullets(8, vecShootOrigin, vecShootDir, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, this->GetHL2MPWpnData().m_iPlayerDamage);
}
...
{
	if (!HasAnimEvent(GetSequence(), EVENT_WEAPON_SHOTGUN_FIRE))
	{
		animevent_t animevent;
		animevent.event = EVENT_WEAPON_SHOTGUN_FIRE;
