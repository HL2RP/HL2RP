...
class CLogicAtmos : public CLogicalEntity
{
	...
	byte ComputeLightValue(const tm&, float scale, const byte(&range)[EPeakLightTime::_Count]);
...
void CLogicAtmos::UpdateLightValues()
{
	...
	byte alpha = ComputeLightValue(curTime, scale, mLightAlpha), patternRange[]
		= { (byte)*mLightPattern[EPeakLightTime::MidDay].ToCStr(),
		(byte)*mLightPattern[EPeakLightTime::MidNight].ToCStr() };
	mOnLightAlpha.Set(alpha, this, this);

	alpha = ComputeLightValue(curTime, inverseScale, mLightAlpha);
	mOnInverseLightAlpha.Set(alpha, this, this);
